...
export function ImportDropOverlay({ visible }: ImportDropOverlayProps) {
  ...
  return (
    ...
    >
      <div className="border-2 border-dashed border-primary/70 rounded-2xl
                      p-12 text-center bg-brand-ink/80 shadow-2xl
                      transform scale-100 transition-transform">
        <Upload className="h-16 w-16 text-primary/80 mx-auto mb-4" />
        <p className="text-xl font-medium text-white">Drop audio file to import</p>
        <p className="text-sm text-brand-eraser mt-2">{getAudioFormatsDisplayList()}</p>
      </div>
    </div>
  );
}
